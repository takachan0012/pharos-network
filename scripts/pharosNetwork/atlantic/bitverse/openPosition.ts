...
export async function openPosition({
  ...
  let priceParseDecimalsToken = Number(
    parseUnits(price, decimalsToken)
  ).toFixed(2);
  if (orderType == 2) {
    priceParseDecimalsToken = Number(
      (parseUnits(price, decimalsToken) * 80n) / 100n
    ).toFixed(2);
  }
  const amount = Number(
    randomAmount({
  ...
  const params = [
    ...
    100000n,
    [[contractAddress, parseUnits(margin[0].amount, decimalsToken)]],
    0n,
    0n,
    BigInt(longOI),
    BigInt(shortOI),
